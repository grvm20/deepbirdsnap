"""
Train entire model to learn bounded box
"""
import gc
...
def defrost_all_parts():
    ...
    model = create_model(num_classes=4,activation=None,weights=base_weights, weights_output_dim=500, top_weights=top_weights) 
    ...
    for i in range(epochs):
        ...
        train_generator = utils.img_parts_generator(part_file_name, train_data_dir, batch_size=5000, load_image=True, unpickled=unpickled_train, bb_only=True)
        val_generator = utils.img_parts_generator(part_file_name, validation_data_dir, batch_size=3000, load_image=True, unpickled=unpickled_valid, bb_only=True)
