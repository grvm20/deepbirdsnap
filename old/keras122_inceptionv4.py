...
TH_WEIGHTS_PATH = 'https://github.com/kentsommer/keras-inceptionV4/releases/download/2.0/inception-v4_weights_th_dim_ordering_th_kernels_notop.h5'
TF_WEIGHTS_PATH = 'https://github.com/kentsommer/keras-inceptionV4/releases/download/2.0/inception-v4_weights_tf_dim_ordering_tf_kernels_notop.h5'
...
def inception_v4(num_classes, dropout_keep_prob, weights, include_top=True):
    '''
    ...
    if include_top:    
        ...
        # 1536
        net = Dense(output_dim=num_classes, activation='softmax')(net)
    
    model = Model(inputs, net, name='inception_v4')
    
    # load weights
    if include_top:
        model.load_weights('models/inception-v4_weights_tf_dim_ordering_tf_kernels.h5')#, by_name=True)
    else:
        model.load_weights('models/inception-v4_weights_tf_dim_ordering_tf_kernels_notop.h5')#, by_name=True)
...
def create_model(num_classes=1001, dropout_keep_prob=0.2, weights='imagenet', include_top=True):
	return inception_v4(num_classes, dropout_keep_prob, weights, include_top)
